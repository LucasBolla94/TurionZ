...
**Tarefas:**
- [ ] 0.1: `npm init` — criar `package.json` (name: "turionz", scripts: dev/build/start)
- [ ] 0.2: Instalar TypeScript + ts-node + tsx + nodemon + @types/node
- [ ] 0.3: Criar `tsconfig.json` (strict mode, target ES2022, module NodeNext)
- [ ] 0.4: Criar toda a estrutura de pastas conforme diagrama acima
- [ ] 0.5: Criar `.gitignore` (node_modules, data/, tmp/, .env, vault.key, vault.enc)
- [ ] 0.6: Criar `.env.example` com todas as variáveis
- [ ] 0.7: Criar `src/types/index.ts` com interfaces base:
...
- [ ] 0.8: Criar `src/index.ts` com hello world básico
